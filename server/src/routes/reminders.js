  ...
  const linked = new Set(
    query(
      `SELECT DISTINCT enrollment_id FROM reminder_logs
       WHERE substr(sent_at, 1, 7) = ? AND status != 'error'`,
      monthKey,
...
router.post('/send', async (req, res) => {
  try {
    ...
    if (ids.length) {
      targets = ids.map(getEnrollmentWithFees).filter(Boolean);
    } else {
      targets = fetchEnrollments({ status: 'active', due_only: true });
    }
...
router.get('/logs', (req, res) => {
  ...
  if (/^\d{4}-\d{2}$/.test(month)) {
    conds.push('substr(rl.sent_at, 1, 7) = ?');
    params.push(month);
  }
  const enrollmentId = Number(req.query.enrollment_id);
  if (enrollmentId) {
    conds.push('rl.enrollment_id = ?');
