...
/**
 * Mask a phone number for the public receipt. `/share/r/:token` is unauthenticated - anyone
 * holding the link can open it - so the shared view shows only the country code and last 4
 * digits. Staff still get the real number in the CRM and on the internal receipt
 * (`receiptHtml(r)` without `shared`).
 */
...
<div class="wrap">
  ...
  <div class="receipt">
    ...
    <div class="r-body">
      ...
      <div class="grid">
        <div class="cell"><span class="k">Student ID</span><span class="v">${escapeHtml(r.studentRef)}</span></div>
        <div class="cell"><span class="k">Student name</span><span class="v">${escapeHtml(r.studentName)}</span></div>
        <div class="cell"><span class="k">Parent</span><span class="v">${escapeHtml(r.guardian)}</span></div>
        <div class="cell"><span class="k">Mobile no.</span><span class="v">${escapeHtml(shared ? maskMobile(r.mobile) : r.mobile)}</span></div>
      </div>
    ...
    <div class="r-foot">
      <p>Thank you for nurturing <b>${escapeHtml(r.studentName)}'s</b> creative journey at <b>Pravaha Art Space</b>.</p>
